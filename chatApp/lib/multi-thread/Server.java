...
public class Server {
    static Vector<Client_Handler> sockets = new Vector<Client_Handler>(); // Vector to hold the each instance of
                                                                          // class client_handler(using vectors instead
                                                                          // of lists because vectors are thread safe)
    ...
    public static void main(String args[]) throws IOException {
        ...
        while (true) {
            ...
            BufferedReader input = new BufferedReader(new InputStreamReader(connection.getInputStream()));// INPUT
                                                                                                          // STREAM
            ...
            while (true) {
                ...
                if (msg.endsWith("Client_wants_to_end_the_connection")) {
                    ...
                    for (Client_Handler c : Server.sockets) {
                        if (!c.client_name.equals(client_name))
                            c.Send(msg);
                    }
                    CloseStreams();
                    break;
